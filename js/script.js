...
document.querySelectorAll(".gator").forEach((gator) => {
  gator.onclick = function () {
    ...
    //Media query. Escape navbar on mobile and tablets onclick on the navlinks
    const mq = window.matchMedia("(max-width: 1199px)");
    if (mq.matches) {
      navTransform("translate(-100%)", "visible");
    }

    //Navbar link colors onclick event located at the right side bar
    sideColor.forEach((sidebtn) => {
      sidebtn.addEventListener("click", () => {
        if (gator.classList.contains("current")) {
          gator.style.background = sidebtn.dataset.color;
        } else {
          gator.style.background = "transparent";
        }
...
function navAnim(a, b, c, d) {
  nav.firstElementChild.style.width = a;
  nav.firstElementChild.style.paddingRight = b;
  body.style.width = c;
  body.style.left = d;
}
